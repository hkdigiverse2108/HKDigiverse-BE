...
import { apiResponse, IForgotPasswordValidate, ILoginValidate, IRegisterValidate, IResendOtpValidate, IResetPasswordValidate, IUpdatePasswordValidate, IVerifyOtpValidate } from "../../type";
...
export const register = async (req, res) => {
  ...
  try {
    ...
    return res.status(HTTP_STATUS.OK).json(apiResponse(HTTP_STATUS.OK, responseMessage?.signupSuccess, response, {}));
  } catch (error) {
    console.error(error);
...
export const login = async (req, res) => {
  ...
  try {
    ...
    return res.status(HTTP_STATUS.OK).json(apiResponse(HTTP_STATUS.OK, responseMessage?.loginSuccess, response, {}));
  } catch (error) {
    console.error(error);
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, responseMessage?.internalServerError, {}, error));
  }
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    return res.status(HTTP_STATUS.OK).json(apiResponse(HTTP_STATUS.OK, responseMessage?.resendOtpSuccess, {}, {}));
  } catch (error) {
    console.error(error);
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, responseMessage?.internalServerError, {}, error));
  }
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    const comparePassword = compareHash(value?.oldPassword, response?.password);
    if (!comparePassword) return res.status(HTTP_STATUS.BAD_REQUEST).json(apiResponse(HTTP_STATUS.BAD_REQUEST, responseMessage?.invalidPassword, {}, {}));
...
export const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    if (response?.otpExpireTime < new Date()) return res.status(HTTP_STATUS.BAD_REQUEST).json(apiResponse(HTTP_STATUS.BAD_REQUEST, responseMessage?.expireOTP, {}, {}));

    ["password", "otp", "otpExpireTime"].forEach((key) => delete response[key]);

    return res.status(HTTP_STATUS.OK).json(apiResponse(HTTP_STATUS.OK, responseMessage?.OTPVerified, response, {}));
  } catch (error) {
    console.error(error);
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, responseMessage?.internalServerError, {}, {}));
  }
